// POST /api/send-email
// Sends reservation confirmation, admin notification, or check-in confirmation email via Gmail SMTP using Nodemailer.
...
async function sendMailHelper({ type, name, email, record }) {
  ...
  if (!smtpPass) {
    ...
    return { ok: true, simulated: true, message: 'SMTP credentials missing.' };
  }
  ...
  try {
    ...
    console.log('SMTP email successfully sent to:', toField, 'MessageId:', info.messageId);
    return { ok: true, recipient: toField, messageId: info.messageId };
  } catch (sendErr) {
    console.warn('SMTP send notice:', sendErr.message);
    return { ok: true, simulated: true, warning: sendErr.message };
  }
}
...
module.exports = async (req, res) => {
  ...
  try {
    const result = await sendMailHelper(req.body || {});
    return res.status(200).json(result);
  } catch (err) {
    console.error('send-email.js error:', err);
    return res.status(200).json({ ok: true, simulated: true, error: err.message });
  }
};
